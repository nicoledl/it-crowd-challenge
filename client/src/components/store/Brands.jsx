...
const Brands = () => {
  ...
  const onClick = (name) => {
    window.location.href = `${process.env.NEXT_CLIENT_URL_BASE}/store/search/page?keyword=${name}`;
  };
  ...
  return (
    <>
      ...
      <Pagination setPage={setPage} url={`${process.env.NEXT_SERVER_URL_BASE}/brands/`} />
    </>
  );
};
